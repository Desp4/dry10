...
namespace vkw
{
        ...
        _frameCount(imageCount)
    {
        ...
        vkGetSwapchainImagesKHR(_device->device(), _swapchain, &imageCount, _swapImages.data());
    ...
    {
        vkWaitForFences(_device->device(), 1, _inFlightFences.data() + _currentFrame, VK_TRUE, UINT64_MAX);

        uint32_t imageIndex;
        vkAcquireNextImageKHR(_device->device(), _swapchain, UINT64_MAX, _imageAvailableSemaphores[_currentFrame], VK_NULL_HANDLE, &imageIndex);
    ...
    void PresentSwapchain::submitFrame(VkQueue queue, PresentFrame frame, const VkCommandBuffer* cmdBuffer)
    {
        ...
        vkResetFences(_device->device(), 1, _inFlightFences.data() + frame.frameIndex);
