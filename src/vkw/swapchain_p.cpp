...
vk_swapchain_present::vk_swapchain_present(const vk_device& device,
    ...
    _device{ &device }
{
    ...
    vkCreateSwapchainKHR(_device->handle(), &swapchain_info, null_alloc, &_swapchain);

    // Create images and their fences
    vkGetSwapchainImagesKHR(_device->handle(), _swapchain, &_frame_count, nullptr);
    ...
    vkGetSwapchainImagesKHR(_device->handle(), _swapchain, &_frame_count, _swap_images.data());
    for (auto i = 0u; i < _frame_count; ++i) {
        _swap_image_views[i] = vk_image_view{ *_device, _swap_images[i], format, 1, VK_IMAGE_ASPECT_COLOR_BIT };
    }
    ...
    for (auto i = 0u; i < _frame_count; ++i) {
        vkCreateSemaphore(_device->handle(), &semaphore_info, null_alloc, _image_available_semaphores.data() + i);
        vkCreateSemaphore(_device->handle(), &semaphore_info, null_alloc, _render_finished_semaphores.data() + i);
        vkCreateFence(_device->handle(), &fence_info, null_alloc, _in_flight_fences.data() + i);
    }
}
...
vk_swapchain_present::~vk_swapchain_present() {
    if (_device != nullptr) {
        for (auto i = 0u; i < _frame_count; ++i) {
            vkDestroySemaphore(_device->handle(), _image_available_semaphores[i], null_alloc);
            vkDestroySemaphore(_device->handle(), _render_finished_semaphores[i], null_alloc);
            vkDestroyFence(_device->handle(), _in_flight_fences[i], null_alloc);
        }
        vkDestroySwapchainKHR(_device->handle(), _swapchain, null_alloc);
    }
}

u32_t vk_swapchain_present::acquire_frame() {
    vkWaitForFences(_device->handle(), 1, _in_flight_fences.data() + _next_frame, VK_TRUE, UINT64_MAX);

    // _nextFrame and imageIndex are the same after this call
    u32_t image_index = 0;
    vkAcquireNextImageKHR(_device->handle(), _swapchain, UINT64_MAX, _image_available_semaphores[_next_frame], VK_NULL_HANDLE, &image_index);
    ...
    if (_in_flight_image_fences[image_index] != VK_NULL_HANDLE) {
        vkWaitForFences(_device->handle(), 1, _in_flight_image_fences.data() + image_index, VK_TRUE, UINT64_MAX);
    }
...
void vk_swapchain_present::submit_frame(VkQueue queue, u32_t frame_index, const VkCommandBuffer& cmd_buf) {
    ...
    vkResetFences(_device->handle(), 1, _in_flight_fences.data() + frame_index);
