...
{
    _pools.push_back(vkw::vk_descriptor_pool{sizes, capacity});
    _pools.back().create_sets(_available_sets, layout);
}

VkDescriptorSet descriptor_pool_pool::get_descriptor_set() {
    if (_available_sets.size() == 0) {
        _pools.push_back(vkw::vk_descriptor_pool{ _sizes, _capacity });
        _available_sets.resize(_capacity);
        _pools.back().create_sets(_available_sets, _layout);
    }
...
void descriptor_pool_pool::update_descriptor_set(VkDescriptorSet desc_set, std::span<VkWriteDescriptorSet> desc_writes) const {
    ...
    vkUpdateDescriptorSets(vkw::g_device->handle(), static_cast<uint32_t>(desc_writes.size()), desc_writes.data(), 0, nullptr);
}
